...
const getEmailTemplate = (type: string, data: Record<string, any>): { subject: string; html: string } => {
  ...
  switch (type) {
        ...
        html: `<!DOCTYPE html><html><head><meta charset="utf-8"></head>
          <body style="${baseStyles} max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="${contentStyle}">
              ...
              <p style="color: #6B7280; font-size: 14px;">Email automático do sistema NODOC.</p>
            </div>
            <div style="text-align: center; padding: 16px; color: #9CA3AF; font-size: 12px;"><p>© ${new Date().getFullYear()} NODOC</p></div>
          </body></html>`
      };
    ...
    default:
      return { subject: 'Notificação NODOC', html: `<p>Nova notificação no sistema NODOC.</p>` };
  }
};
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const emailResponse = await resend.emails.send({
      from: "NODOC <onboarding@resend.dev>",
      to: [email],
      subject,
      html,
    });
