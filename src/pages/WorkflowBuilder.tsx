...
import { useWorkflows, WorkflowNodeData, WorkflowConnectionData } from "@/hooks/useWorkflows";
...
const WorkflowBuilder = () => {
  ...
  const handleNodeClick = (e: React.MouseEvent, node: WorkflowNodeData) => {
    e.stopPropagation();
  ...
  const handleNodeMouseDown = (e: React.MouseEvent, node: WorkflowNodeData) => {
  ...
  const handleCanvasClick = () => {
    if (selectedTool === "connect") setConnectingFrom(null);
  ...
  const saveWorkflow = async () => {
    if (currentWorkflowId) {
      updateWorkflow.mutate({ id: currentWorkflowId, name: workflowName, nodes, connections });
    } else {
      createWorkflow.mutate(
        { name: workflowName, nodes, connections },
        {
          onSuccess: (data: any) => {
  ...
  // Load a workflow
  const loadWorkflow = (wf: typeof workflows[0]) => {
    setCurrentWorkflowId(wf.id);
    setWorkflowName(wf.name);
    setNodes(wf.nodes);
    setConnections(wf.connections);
    ...
    toast.success(`Workflow "${wf.name}" carregado`);
  };
  ...
  const validateWorkflow = () => {
    ...
    if (disconnectedNodes.length > 0) errors.push(`${disconnectedNodes.length} nó(s) não conectado(s)`);

    const unnamedTasks = nodes.filter(n => n.type === "task" && (!n.name || n.name === "Nova Tarefa"));
    if (unnamedTasks.length > 0) errors.push(`${unnamedTasks.length} tarefa(s) sem nome definido`);
  ...
  const renderNode = (node: WorkflowNodeData) => {
    ...
    let size = "w-20 h-20";
    switch (node.type) {
      case "start": nodeStyle = "rounded-full bg-success/20 border-2 border-success"; size = "w-16 h-16"; break;
      case "end": nodeStyle = "rounded-full bg-destructive/20 border-2 border-destructive"; size = "w-16 h-16"; break;
      case "task": nodeStyle = "rounded-lg bg-primary/10 border-2 border-primary"; size = "w-32 h-20"; break;
      case "gateway": nodeStyle = "rotate-45 bg-warning/20 border-2 border-warning"; size = "w-14 h-14"; break;
    }
    ...
    return (
      <div
        ...
        style={{ left: node.x, top: node.y, transform: node.type === "gateway" ? "rotate(45deg)" : undefined }}
        onClick={(e) => handleNodeClick(e, node)}
        onMouseDown={(e) => handleNodeMouseDown(e, node)}
      >
        <div className={cn("flex flex-col items-center gap-1", node.type === "gateway" && "-rotate-45")}>
          <Icon className={cn(
            "h-5 w-5",
  ...
  const renderConnection = (connection: WorkflowConnectionData) => {
    ...
    const adjustedFrom = { x: from.x + (to.x - from.x) * ratio, y: from.y + (to.y - from.y) * ratio };
    const adjustedTo = { x: to.x - (to.x - from.x) * ratio, y: to.y - (to.y - from.y) * ratio };

    return (
      <g key={connection.id}>
        <line
          x1={adjustedFrom.x} y1={adjustedFrom.y}
          x2={adjustedTo.x} y2={adjustedTo.y}
          stroke="hsl(var(--muted-foreground))" strokeWidth="2" markerEnd="url(#arrowhead)"
        />
      </g>
    );
  };
  ...
  return (
    <DashboardLayout title="Construtor de Workflow" subtitle="Desenhe workflows BPMN com arrastar e soltar">
      <PageBreadcrumb items={[{ label: "Processos", href: "/processes" }, { label: "Construtor de Workflow" }]} />
          ...
          <CardContent className="p-3 space-y-3">
            ...
            <div className="pt-3 border-t border-border">
              ...
              <div className="space-y-2">
                <Button variant={selectedTool === "select" ? "default" : "outline"} size="sm" className="w-full justify-start"
                  onClick={() => { setSelectedTool("select"); setConnectingFrom(null); }}>
                  <MousePointer className="h-4 w-4 mr-2" /> Seleccionar
                </Button>
                <Button variant={selectedTool === "connect" ? "default" : "outline"} size="sm" className="w-full justify-start"
                  onClick={() => setSelectedTool("connect")}>
                  <ArrowRight className="h-4 w-4 mr-2" /> Conectar
                </Button>
              </div>
            </div>

            <div className="pt-3 border-t border-border">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Vista</p>
              ...
              <p className="text-xs text-muted-foreground mt-2 text-center">{Math.round(zoom * 100)}%</p>
            </div>

            <div className="pt-3 border-t border-border">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Ficheiro</p>
              <div className="space-y-2">
                <Button variant="outline" size="sm" className="w-full justify-start" onClick={newWorkflow}>
                  <FilePlus className="h-4 w-4 mr-2" /> Novo
                </Button>
                <Button variant="outline" size="sm" className="w-full justify-start" onClick={() => setShowWorkflowList(!showWorkflowList)}>
              ...
              {showWorkflowList && (
                <div className="mt-2 space-y-1 max-h-40 overflow-y-auto">
                  {isLoading ? (
                    ...
                    <p className="text-xs text-muted-foreground text-center py-2">Nenhum workflow guardado</p>
                  ) : (
                    workflows.map(wf => (
                      <button
                        key={wf.id}
                        ...
                        className={cn(
                          ...
                          currentWorkflowId === wf.id && "bg-accent font-medium"
                        )}
                      >
                        {wf.name}
                      </button>
        ...
        <Card className="col-span-7 overflow-hidden flex flex-col">
          <CardHeader className="py-3 px-4 border-b border-border flex-row items-center justify-between">
            <div className="flex items-center gap-3">
              <Input value={workflowName} onChange={(e) => setWorkflowName(e.target.value)} className="w-64 h-8 text-sm font-medium" />
              {currentWorkflowId && <Badge variant="outline" className="text-xs">Guardado</Badge>}
              {connectingFrom && <Badge variant="secondary" className="animate-pulse">Modo de conexão activo</Badge>}
            </div>
          ...
          <CardContent className="p-0 flex-1 overflow-hidden">
            <div
              ...
              style={{ minHeight: "100%", cursor: draggingNode ? "grabbing" : selectedTool === "connect" ? "crosshair" : "default" }}
              onDrop={handleCanvasDrop} onDragOver={handleCanvasDragOver} onClick={handleCanvasClick}
              onMouseMove={handleCanvasMouseMove} onMouseUp={handleCanvasMouseUp} onMouseLeave={handleCanvasMouseUp}
            >
              <div style={{ transform: `scale(${zoom})`, transformOrigin: "top left", minWidth: "1500px", minHeight: "1000px", position: "relative" }}>
                <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ minWidth: "1500px", minHeight: "1000px" }}>
                  <defs>
                ...
                {nodes.length === 0 && (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <p className="text-lg font-medium text-muted-foreground">Arraste elementos da caixa de ferramentas</p>
                      <p className="text-sm text-muted-foreground">para começar a construir o seu workflow</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            {selectedNode ? (
              <div className="space-y-4">
                <div className="flex items-center gap-3 pb-4 border-b border-border">
                  ...
                  <div>
                    <p className="font-medium">{nodeTypes.find(n => n.type === selectedNode.type)?.label}</p>
                    <p className="text-xs text-muted-foreground font-mono">{selectedNode.id.slice(0, 15)}...</p>
                  </div>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="nodeName">Nome</Label>
                  <Input id="nodeName" value={selectedNode.name} onChange={(e) => updateSelectedNode({ name: e.target.value })} />
                </div>
                ...
                {selectedNode.type === "task" && (
                  <>
                    <div className="space-y-2">
                      ...
                      <Select value={selectedNode.taskType || "manual"} onValueChange={(v) => updateSelectedNode({ taskType: v })}>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input id="assignee" placeholder="Nome do responsável" value={selectedNode.assignee || ""}
                        onChange={(e) => updateSelectedNode({ assignee: e.target.value || undefined })} />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="sla">SLA (dias)</Label>
                      <Input id="sla" type="number" min="1" placeholder="Ex: 5" value={selectedNode.sla || ""}
                ...
                {selectedNode.type === "gateway" && (
                  <div className="space-y-2">
                    ...
                    <Textarea id="condition" placeholder="Ex: valor > 10000" rows={3} value={selectedNode.condition || ""}
                      onChange={(e) => updateSelectedNode({ condition: e.target.value })} />
                    <p className="text-xs text-muted-foreground">Defina a condição para a decisão</p>
                  </div>
                )}

                <div className="space-y-2">
                  <Label htmlFor="description">Descrição</Label>
                  <Textarea id="description" placeholder="Descrição opcional..." rows={3} value={selectedNode.description || ""}
                    onChange={(e) => updateSelectedNode({ description: e.target.value })} />
                </div>

                <div className="pt-4 border-t border-border space-y-2">
                  <Button variant="outline" size="sm" className="w-full" onClick={duplicateSelectedNode}>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm text-muted-foreground">Seleccione um elemento para editar as suas propriedades</p>
              </div>
            )}
          </CardContent>
        </Card>
